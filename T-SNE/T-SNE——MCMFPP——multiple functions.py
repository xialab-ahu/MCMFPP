...
def get_config():
    ...
    # Classification Task
    parse.add_argument('-peptides_name', type=list, default=['AAP', 'ABP', 'ACP.txt', 'ACVP', 'ADP', 'AEP', 'AFP', 'AHIVP',
    ...
    parse.add_argument('-vocab_size', type=int, default=21, help='Vocabulary size, i.e., the total number of vocabulary types')
    parse.add_argument('-position_embedding_dim', type=int, default=128, help='Embedding dimension for positional encoding word vectors (128). If set to 0, positional encoding is not performed')
    parse.add_argument('-ESMC_embedding_dim', type=int, default=960, help='0 or 960. 0 indicates that ESMC extraction is not used')
    parse.add_argument('-n_filters', type=int, default=128, help='Number of filters for each convolutional kernel, i.e., the number of channels representing sample word features after convolution')
    parse.add_argument('-filter_sizes', type=list, default=[3, 4, 5], help='Sizes of convolutional kernels. Each element in the list represents the size of a convolutional kernel')
    parse.add_argument('-dropout', type=float, default=0.6, help='Dropout rate to prevent overfitting')
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            for k in tail_label:
                preds_value[:, k] = 0.4 * preds1_value[:, k] + 0.6 * preds2_value[:, k]
    ...
    case_labels =  ['THP', 'ADP', 'DPPIP', 'BBP', 'AHP', 'AIP',
                    ...
                    'APP', 'AFP', 'QSP', 'ABP_AFP', 'ABP_ACP', 'ACVP', 'AVP']
    case_colors =  [
    'black', 'black', '#2ca02c', 'black', '#9467bd', 'black',
    'black', '#7f7f7f', 'black', 'black', 'black', 'black',
    'black', '#ff9896', 'black', '#c49c94', 'black', '#c7c7c7',
    'black', '#9edae5', 'black', 'black', 'black', 'black','black'
]
    tag_labels1= ['DPPIP','AHP','ACP','ABP_ACP_AFP', 'ABP','AHP_DPPIP', 'AFP','Others']
    tag_colors1 =  ['#2ca02c','#9467bd','#7f7f7f','#ff9896','#c49c94','#c7c7c7', '#9edae5','black']
    ...
    if tag_labels1:
        ...
        savepath = f'../Figures/mcmfpp_multiple_TSNE.png'
        ...
        for j, (label, func_label) in enumerate(
            ...
            if func_label=='DPPIP':
                ...
                text_x = center[0] - 5
                text_y = center[1] - 8
            elif func_label=='AHP_DPPIP':
                text_x = center[0] + 10
                text_y = center[1] - 10
            elif func_label=='ABP_ACP_AFP':
                text_x = center[0] - 1
                text_y = center[1] - 5
            else:
                text_x = center[0] - 5
